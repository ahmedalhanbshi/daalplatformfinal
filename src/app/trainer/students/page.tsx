...
function TrainerStudentsRegistrationsPageContent() {
  ...
  const load = useCallback(async () => {
    try {
      setLoading(true)
      const [s, e] = await Promise.all([trainerService.getAllStudents(), trainerService.getEnrollments()])
  ...
  const submitReasonAction = async () => {
    ...
    await withAction(targetId, async () => {
      await trainerService.updateEnrollmentStatus(targetId, "CANCELLED", reasonMode === "stop" ? `إيقاف: ${reason}` : `إلغاء: ${reason}`)
    })
      ...
      <RejectionActionModal
        ...
        onConfirm={async (reason) => {
          ...
          await withAction(rejectTarget.id, async () => {
            await trainerService.updateEnrollmentStatus(rejectTarget.id, rejectType === "payment" ? "REJECT_PAYMENT" : "REJECT_ENROLLMENT", reason)
          })
...
export default function TrainerStudentsRegistrationsPage() {
  return (
    <Suspense
      fallback={
        <div className="flex min-h-[50vh] items-center justify-center text-sm text-slate-500" dir="rtl">
          جاري التحميل...
        </div>
      }
    >
      <TrainerStudentsRegistrationsPageContent />
    </Suspense>
  )
}
