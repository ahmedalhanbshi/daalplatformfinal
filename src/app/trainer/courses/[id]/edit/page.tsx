        ...
        const fetchData = async () => {
            try {
                ...
                setCourseData({
                    ...
                    deliveryType: (course.deliveryType || "").toLowerCase(),
                ...
                if (Array.isArray(course.sessions) && course.sessions.length > 0) {
                    if ((course.deliveryType || "").toLowerCase() === "in_person") {
                        ...
                        const toDateOnly = (session: any) => {
                            if (session.date && typeof session.date === "string") return session.date.slice(0, 10)
                            if (session.startTime) return new Date(session.startTime).toISOString().slice(0, 10)
                            return ""
                        }
                        ...
                        const mappedOnline = course.sessions.map((s: any) => {
                            const startRaw = s.startTime || ""
                            const endRaw = s.endTime || ""
                            const dateOnly = toDateOnly(s)
                            const startTimeOnly = toTimeOnly(startRaw)
                            const endTimeOnly = toTimeOnly(endRaw)
                            const start = startTimeOnly && dateOnly ? new Date(`${dateOnly}T${startTimeOnly}`) : null
                            const end = endTimeOnly && dateOnly ? new Date(`${dateOnly}T${endTimeOnly}`) : null
                        ...
                        if (mappedOnline.length > 0) setOnlineSessions(mappedOnline)
                        ...
                        <DialogContent dir="rtl">
                            ...
                            <DialogFooter className="flex-row-reverse gap-2 sm:justify-start">
                                <Button variant="destructive" onClick={handleDeleteCourse} disabled={isSubmitting}>
                                    {isSubmitting ? <Loader2 className="h-4 w-4 animate-spin" /> : "حذف نهائي"}
                                </Button>
