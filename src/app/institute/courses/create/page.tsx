...
function CreateCoursePageInner() {
    ...
    const editCourseId = searchParams.get("editId") || ""
    ...
    // Fetch Data + optional edit payload
    useEffect(() => {
        const fetchData = async () => {
            try {
                ...
                setInstituteIds(ids)
                setInstituteName((profile?.name || profile?.institute?.name || "").trim())
                setCategories(cats)
                setTrainers(trns)
                setHalls(hls)
                ...
                if (isEditMode && editCourseId) {
                    ...
                    setCourseData({
                        ...
                        deliveryType: (existing.deliveryType || "").toLowerCase(),
                        price: existing.price?.toString() || "",
                        minStudents: existing.minStudents?.toString() || "",
                        maxStudents: existing.maxStudents?.toString() || "",
                    ...
                    } else if (existing.trainer?.id) {
                        setSelectedTrainerIds([existing.trainer.id])
                    }

                    if (existing.image) {
                        const img = getFileUrl(existing.image)
                        if (img) setImagePreview(img)
                    }

                    if (existing.deliveryType === "online" && existing.sessions?.length > 0) {
                        const loadedOnline = existing.sessions.map((s: any) => {
                            const start = s.startTime ? new Date(s.startTime) : null
                            const end = s.endTime ? new Date(s.endTime) : null
                            const durationMins =
                                start && end ? Math.round((end.getTime() - start.getTime()) / 60000).toString() : "60"
                            return {
                ...
                toast.error("فش�„ ف�Š تحميل البيانات الأساسية")
                console.error(err)
            } finally {
                setLoading(false)
            }
    ...
    const mappedHalls = halls.map(h => ({
        id: h.id,
        instituteId: h.institute?.id || "",
        instituteName: h.institute?.name || "",
        name: h.name,
    ...
    const isOwnInstituteHall = !!selectedHall && (
        // If hall owner id exists, compare against any institute id from profile payload.
        (selectedHall.instituteId ? instituteIds.includes(selectedHall.instituteId) : true) ||
        // Fallback for APIs that return owner name without stable id.
        (!!selectedHall.instituteName && !!instituteName && normalize(selectedHall.instituteName) === normalize(instituteName))
    )
        ...
        try {
            ...
            if (status === 'DRAFT' || status === 'PENDING_MINIMUM') {
                ...
                if (status === 'ACTIVE') {
                    ...
                    if (!isOwnInstituteHall && !paymentFile) throw new Error("يجب إرفا�‚ سند ا�„دفع لحجز القاعة");
                }
            ...
            if (isEditMode && editCourseId) {
                await instituteService.updateCourse(editCourseId, formData)
            } else {
            ...
            toast.success(
                status === 'DRAFT'
                    ...
                    : status === 'PENDING_MINIMUM'
                        ? (isEditMode ? 'تم تحديث الدورة! ستُفع�‘�„ عند اكتمال الحد الأدنى وإكمال الإعداد' : 'تم نشر الدورة! ستُفع�‘�„ عند اكتمال الحد الأدنى وإكمال الإعداد')
            ...
            const backendMessage = err?.response?.data?.message
            toast.error(backendMessage || err?.message || (isEditMode ? 'حدث خطأ أثناء تحديث الدورة' : 'حدث خطأ أثناء إنشاء الدورة'));
            console.error(err);
        } finally {
            setIsSubmitting(false);
        }
                                        ...
                                        <div className="space-y-5 lg:order-2" dir="rtl">
                                            ...
                                            <div className="space-y-2.5 pr-2 sm:pr-3 lg:pr-4">
                                                ...
                                                <div className="relative">
                                                    <button
                                                        type="button"
                                                        className="w-full rounded-md border border-input bg-background px-3 py-2 text-right text-sm shadow-sm transition-colors hover:border-blue-300"
                                                        onClick={() => setTrainerDropdownOpen(prev => !prev)}
                                                    >
                                                        <span className={selectedTrainerIds.length === 0 ? "text-gray-400" : "text-gray-900"}>
                                                            {selectedTrainerIds.length === 0 ? "اختر مدربًا أو أكثر..." : `${selectedTrainerIds.length} مدربون مختارون`}
                                                        </span>
                                                    </button>
                                                    ...
                                                    {trainerDropdownOpen && (
                                                        <div className="absolute z-20 mt-1 w-full rounded-md border bg-white shadow-lg max-h-56 overflow-auto">
                                                            {trainers.length === 0 ? (
                                                                <p className="p-3 text-sm text-gray-500">لا يوجد مدربون ف�Š الطاقم</p>
                                                            ) : trainers.map((t) => {
                                                                ...
                                                                return (
                                                                    <button
                                                                        key={t.id}
                                                                        type="button"
                                                                        className={`w-full px-3 py-2 text-right text-sm hover:bg-slate-50 flex items-center justify-between ${selected ? "bg-blue-50 text-blue-700" : ""}`}
                                                                        onClick={() => {
                                                            ...
                                                            return (
                                                                <Badge key={id} variant="secondary" className="gap-1">
                                                                    ...
                                                                    <button
                                                                        ...
                                                                        onClick={() => setSelectedTrainerIds(prev => prev.filter(tid => tid !== id))}
                                                                    >
                                                                        <X className="h-3 w-3" />
                                                                    </button>
                                                                </Badge>
                            ...
                            <div className="space-y-4 xl:hidden">
                                ...
                                <article className="group flex flex-col overflow-hidden rounded-lg border border-slate-200/90 bg-white shadow-[0_10px_24px_rgba(15,23,42,0.06)]">
                                    <div className="relative h-[188px] overflow-hidden bg-slate-100 md:h-[196px]">
                                        {imagePreview ? (
                                            <Image src={imagePreview} alt={previewTitle} fill unoptimized className="object-cover object-center" />
                                        ) : (
                                            ...
                                            <div className="space-y-4">
                                                <div className="bg-blue-50 p-4 rounded-lg">
                                                    <div className="flex justify-between items-center mb-3">
                                                        ...
                                                        <div className="text-blue-900 font-bold">الإجمالي: {formatNumber(totalPrice)} ر.ي</div>
                                                    </div>
                        ...
                        <div className="space-y-6">
                            <Card>
                                <CardHeader>
                                    ...
                                    <CardDescription>ف�Š الحجز المرن، المعهد مسؤول عن ت�ˆف�Šر القاعات وتوزيع الطلاب لاحقاً. �Š�ƒف�Š تحديد تاريخ البداية ف�‚ط ليتمكن الطلاب من الحجز.</CardDescription>
                                </CardHeader>
                                <CardContent className="max-w-md">
                                    <div className="space-y-2">
                                        ...
                                        {!courseData.startDate && <p className="text-xs text-amber-600">يرجى تحديد تاريخ البداية لنشر الدورة �ˆتفع�Š�„ التسجيل للطلاب.</p>}
                                    </div>
                                </CardContent>
                            </Card>
                        </div>
